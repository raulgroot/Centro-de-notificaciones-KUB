...
/** Strip the `theme-slug| ` prefix that pieces have. */
...
export function PiecesTable({ pieces }: { pieces: PieceMetrics[] }) {
  return (
    <div className="overflow-hidden rounded-md border border-neutral-200 bg-white">
      ...
      {pieces.length === 0 ? (
        <div className="p-12 text-center text-sm text-neutral-500">Sin datos.</div>
      ) : (
        pieces.map((p, i) => (
          <div
            key={`${p.piece}-${p.product}-${i}`}
            className="grid grid-cols-12 gap-4 border-b border-neutral-100 px-4 py-2.5 text-sm last:border-b-0 hover:bg-neutral-50"
          >
            <div className="col-span-5 min-w-0 truncate text-neutral-900" title={p.piece}>
              {cleanPieceName(p.piece)}
            </div>
            <div className="col-span-1 min-w-0 truncate text-xs text-neutral-600">{p.product}</div>
            <div className="col-span-1 text-right tabular-nums">{numberFmt.format(p.sent)}</div>
            <div className="col-span-1 text-right tabular-nums">{numberFmt.format(p.opened)}</div>
            <div className="col-span-1 text-right tabular-nums">{numberFmt.format(p.clicked)}</div>
            <div className="col-span-1 text-right text-emerald-700 tabular-nums">
              {pctFmt.format(p.openRate)}
            </div>
            <div className="col-span-1 text-right text-blue-700 tabular-nums">
